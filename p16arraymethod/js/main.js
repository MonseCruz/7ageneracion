...
const under_fifty = products.filter( (product) => {
    return product.price < 50
})

const fifty = products.filter( (product) => {
    return product.price >= 50
})

const beer_under_fifty = products.filter( (product) => {
    return product.price < 50 && product.type === "beer"
})
...
const dividedPerTwo = products.map((product) => {
...
const ican = products.map((product) =>{
    if((product.price <20) ){
         product.ican = true;
    }
...
const spanish = products.map((product) => {
    if(product.type === "beer"){ product.type = "cerveza";}
    if(product.type === "chip"){ product.type = "papas";}
    return `${product.type} - ${product.description} - $${product.price}`
})

const suma1 = products.reduce(function(total,product){

    if(product.type == "cerveza")  return total + product.price;
        return total
}, 0 )
